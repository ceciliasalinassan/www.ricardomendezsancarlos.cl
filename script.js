...
mainTitle:'Ricardo Méndez',
subtitle:'Más que un club, una familia.',
description:'Noticias, series, fixture, posiciones, galería, socios, auspiciadores y administración privada para publicar información autorizada.',
...
media:[],
fixture:[{date:'25/05/2026',serie:'SERIE HONOR',game:'Ricardo Méndez vs Atlético Juventud',place:'Local',hour:'15:00'}],
standings:emptyStandings(),
sponsors:BUILTIN_SPONSORS,
requests:[]
};

function getData(){
 const saved=localStorage.getItem('clubRMV5');
 if(!saved){localStorage.setItem('clubRMV5',JSON.stringify(defaults));return structuredClone(defaults);}
 const d=JSON.parse(saved);
 if(!d.standings)d.standings=emptyStandings();
 if(!d.sponsors||!d.sponsors.length)d.sponsors=BUILTIN_SPONSORS;
 return d;
}
function saveData(d){localStorage.setItem('clubRMV5',JSON.stringify(d));}
...
function render(){
 const d=getData();
 siteClubName.textContent=d.clubName;
 mainTitle.textContent=d.mainTitle;
 mainSubtitle.textContent=d.subtitle;
 mainDescription.textContent=d.description;
 clubHistory.textContent=d.history;
 clubPresident.textContent=d.president;
 anniversaryText.textContent=d.anniversary;
 metricAnniversary.textContent=(d.anniversary||'12/08').slice(0,5);
 newsGrid.innerHTML=d.news.map(n=>`<article><div class="thumb"></div><h3>${n.title}</h3><p>${n.text}</p><small>${n.date}</small></article>`).join('');
 galleryGrid.innerHTML=d.media.map(m=>`<article>${m.type==='Video'?`<video src="${m.url}" controls></video>`:`<img src="${m.url}">`}<div><h3>${m.title}</h3><p>${m.type}</p></div></article>`).join('');
 fixtureRows.innerHTML=d.fixture.map(f=>`<tr><td>${f.date}</td><td>${f.serie}</td><td>${f.game}</td><td>${f.place}</td><td>${f.hour}</td></tr>`).join('');
 sponsorGrid.innerHTML=d.sponsors.map(s=>`<article><div class="sponsorLogo"><img src="${s.url}" onerror="this.style.display='none'"></div><h3>${s.name}</h3></article>`).join('');
 renderStandings(); renderRequests();
}

function renderStandings(){
 const d=getData(); const serie=standingSerieSelect.value;
...
openImport.onclick=()=>{openModal();setTimeout(()=>document.querySelector('[data-tab="tabPosiciones"]').click(),100);};
...
openAdmin.onclick=openModal; openAdminTop.onclick=openModal; closeAdmin.onclick=()=>adminModal.classList.remove('show');
loginBtn.onclick=()=>{if(adminPassword.value!==ADMIN_PASS)return alert('Clave incorrecta');loginPanel.classList.add('hidden');adminPanel.classList.remove('hidden');const d=getData();editClubName.value=d.clubName;editMainTitle.value=d.mainTitle;editSubtitle.value=d.subtitle;editDescription.value=d.description;anniversaryInput.value=d.anniversary;historyInput.value=d.history;presidentInput.value=d.president;};
...
saveGeneral.onclick=()=>{const d=getData();d.clubName=editClubName.value;d.mainTitle=editMainTitle.value;d.subtitle=editSubtitle.value;d.description=editDescription.value;d.anniversary=anniversaryInput.value;saveData(d);render();alert('Información general actualizada.');};
...
addMedia.onclick=async()=>{if(!mediaTitle.value)return alert('Agrega título.');let url=mediaUrl.value;let type=mediaType.value;if(mediaFile.files[0]){url=await fileToDataURL(mediaFile.files[0]);type=mediaFile.files[0].type.startsWith('video')?'Video':'Foto';}if(!url)return alert('Adjunta archivo o pega URL.');const d=getData();d.media.unshift({title:mediaTitle.value,type,url});saveData(d);mediaTitle.value='';mediaUrl.value='';mediaFile.value='';render();};
...
importCsv.onclick=async()=>{if(!positionsCsv.files[0])return alert('Adjunta una planilla CSV.');const text=await positionsCsv.files[0].text();const d=getData();text.split(/\r?\n/).slice(1).forEach(line=>{if(!line.trim())return;const [serie,equipo,pj,pg,pe,pp,dg,pts]=line.split(',').map(x=>x?.trim());if(!serie||!equipo)return;if(!d.standings[serie])d.standings[serie]=[];d.standings[serie].push({team:equipo,pj:pj||0,pg:pg||0,pe:pe||0,pp:pp||0,dg:dg||'0',pts:pts||0});});saveData(d);render();alert('Planilla cargada automáticamente.');};
addSponsor.onclick=async()=>{if(!sponsorName.value)return alert('Ingresa nombre del auspiciador.');let url=sponsorUrl.value;if(sponsorFile.files[0])url=await fileToDataURL(sponsorFile.files[0]);if(!url)return alert('Adjunta imagen o pega URL.');const d=getData();d.sponsors.unshift({name:sponsorName.value,url});saveData(d);sponsorName.value='';sponsorUrl.value='';sponsorFile.value='';render();};
